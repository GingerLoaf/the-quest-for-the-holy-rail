Shader "HolyRail/MatrixRain_Billboard"
{
    Properties
    {
        ...
        _Scale ("Character Scale", Range(0.5, 4.0)) = 2.0
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            {
                // Use UV directly for billboards (flat quads)
                // Scale UVs to create more character cells
                float2 fragCoord = input.uv * 256.0;
                float2 resolution = float2(256.0, 256.0);

                // Add per-instance offset to create variety
                float instanceOffset = frac(sin(input.instanceID * 12.9898) * 43758.5453) * 100.0;
                fragCoord.x += instanceOffset;
                fragCoord.y += instanceOffset * 0.7;
